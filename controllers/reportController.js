...
const normalizeMedicinesInput = (payload) => {
  ...
  if (
    ...
    return [
      { medicineId: payload.medicineId, quantity: Number(payload.quantity) },
    ];
  }
...
const createReport = async (req, res) => {
  ...
  const normalizedBalanceSheet = String(balanceSheet || "").trim();
  ...
  if (normalizedMotive.length > 100 || normalizedBalanceSheet.length > 100) {
    return res.status(400).json({
      error: "Le motif et le bilan doivent contenir au maximum 100 caracteres",
    });
  }

  try {
    const medecin = await Medecin.findByPk(normalizedDoctorId);
    if (!medecin) {
    ...
    try {
      const report = await Rapport.create(
        {
          date: normalizedDate,
          motif: normalizedMotive,
          bilan: normalizedBalanceSheet,
...
const updateReport = async (req, res) => {
  ...
  try {
    ...
    if ((report.idVisiteur || "").trim() !== (visiteur.id || "").trim()) {
      return res
        .status(403)
    ...
    if (
      ...
      return res.status(400).json({
        error: "balanceSheet, motive, doctorId et date sont requis",
      });
    }
    ...
    if (normalizedMotive.length > 100 || normalizedBalanceSheet.length > 100) {
      return res.status(400).json({
        error:
          "Le motif et le bilan doivent contenir au maximum 100 caracteres",
      });
    }
